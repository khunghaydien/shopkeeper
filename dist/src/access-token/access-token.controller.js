"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AccessTokenController = void 0;
const common_1 = require("@nestjs/common");
const create_access_token_dto_1 = require("./create-access-token.dto");
const access_token_service_1 = require("./access-token.service");
const client_1 = require("@prisma/client");
let AccessTokenController = class AccessTokenController {
    accessTokenService;
    constructor(accessTokenService) {
        this.accessTokenService = accessTokenService;
    }
    async createAccessToken(data) {
        return this.accessTokenService.createAccessToken(data);
    }
    async getAllAccessTokens() {
        return this.accessTokenService.getAllAccessTokens();
    }
    async getAccessTokenById(id) {
        return this.accessTokenService.getAccessTokenById(Number(id));
    }
    async updateAccessToken(id, data) {
        return this.accessTokenService.updateAccessToken(Number(id), data);
    }
    async deleteAccessToken(id) {
        return this.accessTokenService.deleteAccessToken(Number(id));
    }
};
exports.AccessTokenController = AccessTokenController;
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_access_token_dto_1.CreateAccessTokenDto]),
    __metadata("design:returntype", Promise)
], AccessTokenController.prototype, "createAccessToken", null);
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], AccessTokenController.prototype, "getAllAccessTokens", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], AccessTokenController.prototype, "getAccessTokenById", null);
__decorate([
    (0, common_1.Put)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", Promise)
], AccessTokenController.prototype, "updateAccessToken", null);
__decorate([
    (0, common_1.Delete)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], AccessTokenController.prototype, "deleteAccessToken", null);
exports.AccessTokenController = AccessTokenController = __decorate([
    (0, common_1.Controller)('access-token'),
    __metadata("design:paramtypes", [access_token_service_1.AccessTokenService])
], AccessTokenController);
//# sourceMappingURL=access-token.controller.js.map